...
async function createResourceQuery(resourceData) {
  ...
  const { rows } = await pool.query(
    `INSERT INTO class_resources
     ...
     RETURNING *`,
    [classId, teacherId, title, type, content, description, tags || null, isPublished, expiresAt || null]
  );
  return rows[0];
}
...
async function getResourcesByClassQuery(classId, teacherId) {
  ...
  const query = teacherId
    ? `SELECT * FROM class_resources WHERE class_id = $1`
    : `SELECT * FROM class_resources WHERE class_id = $1 AND is_published = true`;
  const { rows } = await pool.query(query, [classId]);
  return rows;
}

async function getResourceByIdQuery(resourceId) {
  const { rows } = await pool.query(`SELECT * FROM class_resources WHERE id = $1`, [resourceId]);
  return rows[0];
}

async function updateResourceQuery(resourceId, teacherId, updates) {
  const allowedFields = ['title', 'description', 'tags', 'is_published', 'expires_at'];
  const setClauses = [];
  const values = [];
  let i = 1;
