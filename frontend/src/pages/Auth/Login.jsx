...
function SkeletonLoader() {
  return (
    <div className="auth-page" role="status" aria-label="Signing you in, please wait" aria-live="polite">
      ...
      <div style={cardStyle}>
        ...
        <div className="auth-left" style={leftStyle}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <div className="sk-pulse" style={{ width: 36, height: 36, borderRadius: 9, background: 'var(--sb-accent-bg)', marginBottom: 40 }} />
            <div className="sk-pulse" style={{ width: 120, height: 34, borderRadius: 6, background: 'var(--sb-hover)', marginBottom: 14 }} />
            <div className="sk-pulse" style={{ width: 160, height: 14, borderRadius: 4, background: 'var(--sb-hover)' }} />
          </div>
        </div>
        {/* Right */}
        <div className="auth-right" style={{ ...rightStyle, gap: 0 }}>
          <div className="sk-pulse" style={{ width: 120, height: 22, borderRadius: 5, background: 'var(--sb-hover)', marginBottom: 6 }} />
          <div className="sk-pulse" style={{ width: 190, height: 13, borderRadius: 4, background: 'var(--sb-hover)', marginBottom: 44 }} />
          {[1, 2].map(i => (
            <div key={i} style={{ marginBottom: 32 }}>
              <div className="sk-pulse" style={{ width: 80, height: 10, borderRadius: 3, background: 'var(--sb-hover)', marginBottom: 12 }} />
...
function FieldError({ id, msg }) {
  ...
  return (
    <span
      id={id}
      role="alert"
      style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11.5, color: 'var(--sb-danger)', marginTop: 5 }}
    >
    ...
    <div className="auth-left" style={leftStyle}>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        ...
        <h2 style={{
          ...
          {title}
          {titleItalic && (
            <><br /><em style={{ color: 'var(--sb-accent)', fontStyle: 'italic' }}>{titleItalic}</em></>
          )}
        </h2>
        <p style={{ fontSize: 13.5, color: 'var(--sb-text-dim)', lineHeight: 1.65, maxWidth: 200 }}>
          {subtitle}
        </p>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 1 }}>
        {bullets && (
          <ul style={{ listStyle: 'none', marginBottom: 20, display: 'flex', flexDirection: 'column', gap: 8 }}>
            {bullets.map((b, i) => (
              <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--sb-text-secondary)' }}>
                <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--sb-accent)', flexShrink: 0 }} />
                {b}
              </li>
            ))}
          </ul>
...
export default function Login() {
  ...
  const handleEmailBlur    = () => { const m = validateEmail(formData.email); if (m) setFieldErrors(p => ({ ...p, email: m })); };
  const handleEmailKeyDown = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const m = validateEmail(formData.email);
      if (m) { setFieldErrors(p => ({ ...p, email: m })); return; }
  ...
  return (
    <div className="auth-page">
      ...
      {/* Skip link */}
      <a href="#login-email" className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-50 focus:px-4 focus:py-2 focus:rounded-lg focus:text-sm"
      ...
      <div style={cardStyle} role="main">
        <LeftPanel
          ...
          bullets={['Secure & encrypted', 'Personalised experience', 'Progress saved automatically']}
        />
        ...
        <div className="auth-right" style={rightStyle}>
          ...
          <div className="af-1" style={{ marginBottom: 36 }}>
            ...
            <p style={{ fontSize: 13.5, color: 'var(--sb-text-dim)' }}>
              ...
              <Link to="/register" style={{ color: 'var(--sb-accent)', fontWeight: 500, textDecoration: 'none' }}
          ...
          <form onSubmit={handleSubmit} noValidate aria-label="Sign in to your account">
            ...
            <div className="af-2" style={{ marginBottom: 28 }}>
              <FieldLabel htmlFor="login-email" icon={Mail}>Email address</FieldLabel>
              <input
                id="login-email" type="email" name="email"
                className={`ul-input${fieldErrors.email ? ' ul-err' : ''}`}
                placeholder="name@example.com"
                value={formData.email}
                onChange={handleChange} onBlur={handleEmailBlur} onKeyDown={handleEmailKeyDown}
              ...
              {fieldErrors.email
                ...
                : <span id="login-email-hint" style={{ fontSize: 11, color: 'var(--sb-text-dim)', marginTop: 4, display: 'block' }}>Press Enter to move to password</span>
              }
            </div>
            ...
            <div className="af-3" style={{ marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <FieldLabel htmlFor="login-password" icon={Lock}>Password</FieldLabel>
                <Link to="/forgot-password"
                  style={{ fontSize: 11.5, color: 'var(--sb-accent)', textDecoration: 'none', fontWeight: 500 }}
                  onMouseEnter={e => e.target.style.textDecoration = 'underline'}
                  onMouseLeave={e => e.target.style.textDecoration = 'none'}>
                  Forgot password?
                </Link>
              ...
              <FieldError id="login-password-error" msg={fieldErrors.password} />
            </div>
