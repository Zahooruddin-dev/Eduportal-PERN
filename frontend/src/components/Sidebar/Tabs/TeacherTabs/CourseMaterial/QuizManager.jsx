...
export default function QuizManager({ classId, user }) {
	...
	const handleOptionChange = (index, field, value) => {
		...
		newOptions[index][field] =
			field === 'isCorrect' ? !newOptions[index].isCorrect : value;
		setQuestionFormData({
			...questionFormData,
			options: newOptions,
		});
		...
		try {
			...
			const payload = {
				...
				passPercentage: formData.passPercentage
					? parseFloat(formData.passPercentage)
					: null,
				attemptsAllowed: formData.attemptsAllowed
					? parseInt(formData.attemptsAllowed)
					: null,
			};
					...
					>
						...
						>
							<div className='flex-1'>
								...
								>
									{quiz.total_submissions} submission(s) ·{' '}
									{quiz.graded_submissions} graded
								</p>
