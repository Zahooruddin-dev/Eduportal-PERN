...
export default function QuickGradePanel({ classId }) {
		...
		return (
			...
			>
				<p style={{ color: 'var(--color-text-muted)' }}>
					Select a class to view submissions
				</p>
			</div>
		);
	}
	...
	if (quizzes.length === 0) {
		return (
			...
			>
				<div
					className='mb-4 inline-block rounded-2xl p-4'
					style={{
	...
	return (
		<div className='space-y-6'>
			{/* Quiz Selector */}
			<div
				className='rounded-xl border-2 p-4 shadow-sm md:rounded-2xl md:p-5'
				style={{
					borderColor: 'var(--color-border)',
					backgroundColor: 'var(--color-surface)',
			...
			<div className='grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4'>
				<div
					className='rounded-xl border-2 p-4 text-center shadow-sm md:rounded-2xl md:p-5'
					style={{
						borderColor: 'var(--color-border)',
						backgroundColor: 'var(--color-surface)',
						boxShadow: 'var(--shadow-sm)',
				...
				>
					...
					<p
						className='mt-2 text-lg font-bold md:text-2xl'
			...
			{error && (
				<div
					className='rounded-xl border p-4 text-sm md:rounded-2xl md:p-5'
					style={{
			...
			{loading ? (
				...
				>
					...
					<p className='mt-2' style={{ color: 'var(--color-text-muted)' }}>
						Students will appear here once they submit the quiz.
					</p>
				</div>
			) : (
				<div className='space-y-3 md:space-y-4'>
					<div
						className='flex items-center justify-between rounded-xl border-2 p-4 md:rounded-2xl md:p-5'
						style={{
							borderColor: 'var(--color-border)',
							backgroundColor: 'var(--color-surface)',
						}}
						...
						>
							<div className='flex items-start justify-between gap-4'>
								<div className='flex-1 min-w-0'>
									<h4
										className='font-bold md:text-base truncate'
										style={{ color: 'var(--color-text-primary)' }}
									>
									...
									{submission.is_graded ? (
										...
										<div className='flex flex-col items-end gap-1'>
											...
											<button
												onClick={() => {
													window.open(
														`#/quiz/${submission.quiz_id}/submission/${submission.id}`,
														'_blank',
													);
												}}
