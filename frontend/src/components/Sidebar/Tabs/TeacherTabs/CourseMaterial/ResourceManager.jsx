...
export default function ResourceManager({ classId, className, onBack }) {
  ...
  const handleTogglePublish = async (resource) => {
    try {
      ...
      setSuccess(`Resource ${!resource.is_published ? 'published' : 'unpublished'}`);
      fetchResources();
    } catch (err) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-sm text-[var(--color-text-primary)]">Upload File</span>
                </label>
                <label className="flex items-center gap-2">
                  <input
                    type="radio"
                    ...
                    className="text-[var(--color-primary)]"
                  />
                  <span className="text-sm text-[var(--color-text-primary)]">External Link</span>
                </label>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="text-[var(--color-primary)]"
              />
              <label htmlFor="isPublished" className="text-sm text-[var(--color-text-primary)]">
          ...
          {resources.map((res) => (
            ...
            >
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    ...
                    {res.expires_at && new Date(res.expires_at) < new Date() && (
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {res.is_published ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                  <button
              ...
              <div className="text-xs text-[var(--color-text-muted)] mt-2">
                ...
                {res.expires_at && ` • Expires: ${new Date(res.expires_at).toLocaleString()}`}
              </div>
            </div>
          ))}
        </div>
