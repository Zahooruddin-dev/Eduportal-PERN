...
export default function AttachmentManager({ classId, assignmentId }) {
  ...
  if (loading) return <div className="text-sm text-[var(--color-text-muted)]">Loading attachments...</div>;

  return (
    <div className="mt-3 border-t border-[var(--color-border)] pt-3">
      <div className="flex justify-between items-center">
        <h4 className="text-sm font-medium text-[var(--color-text-secondary)]">Attachments</h4>
        <button onClick={() => setShowAdd(!showAdd)} className="text-xs text-[var(--color-primary)] flex items-center gap-1">
          <Plus size={12} /> Add
        </button>
      </div>
      {attachments.length === 0 ? (
        <p className="text-xs text-[var(--color-text-muted)] mt-1">No attachments yet.</p>
      ) : (
      ...
      {showAdd && (
        <div className="mt-2 p-2 border border-[var(--color-border)] rounded-lg bg-[var(--color-input-bg)]">
          <input type="text" placeholder="Title" value={title} onChange={e => setTitle(e.target.value)} className="w-full mb-2 p-1 text-sm border border-[var(--color-border)] rounded" />
          <div className="flex gap-2 mb-2">
            <label className="flex items-center gap-1 text-sm"><input type="radio" value="file" checked={addType === 'file'} onChange={() => setAddType('file')} /> File</label>
            <label className="flex items-center gap-1 text-sm"><input type="radio" value="link" checked={addType === 'link'} onChange={() => setAddType('link')} /> Link</label>
          </div>
          {addType === 'file' ? (
            <input type="file" onChange={e => setFile(e.target.files[0])} className="w-full text-sm" />
          ) : (
            <input type="url" placeholder="URL" value={url} onChange={e => setUrl(e.target.value)} className="w-full p-1 text-sm border border-[var(--color-border)] rounded" />
          )}
          <div className="flex justify-end gap-2 mt-2">
            <button onClick={() => setShowAdd(false)} className="text-xs text-[var(--color-text-muted)]">Cancel</button>
            <button onClick={handleAdd} disabled={uploading} className="text-xs text-[var(--color-primary)]">Add</button>
          </div>
        </div>
      )}
