...
export default function Profile() {
	...
	const roleMinPasswordLength = useMemo(
		() => getRoleMinPasswordLength(role),
		[role],
	);
		...
		const loadTeacherData = async () => {
			...
			try {
				...
				const preferredClassId = String(
					teacherProfile.preferred_class_id || '',
				).trim();
				const preferredGradeLabel = String(
					teacherProfile.preferred_grade_label || '',
				).trim();
				...
				setTeacherForm({
					subjects: Array.isArray(teacherProfile.subjects)
						...
						: '',
					selectedClassId:
						preferredClassId || (preferredGradeLabel ? 'other' : ''),
	...
	const handleTeacherChange = (event) => {
		...
		setTeacherForm((prev) => ({
			...prev,
			[name]: value,
			...(name === 'selectedClassId' && value !== 'other'
				? { otherGrade: '' }
				: {}),
		}));
	};
	...
	const handleTeacherSubmit = async (event) => {
		...
		try {
			...
			if (nextProfile) {
				const preferredClassId = String(
					nextProfile.preferred_class_id || '',
				).trim();
				const preferredGradeLabel = String(
					nextProfile.preferred_grade_label || '',
				).trim();

				setTeacherForm({
					subjects: Array.isArray(nextProfile.subjects)
						? nextProfile.subjects.join(', ')
						: '',
					selectedClassId:
						preferredClassId || (preferredGradeLabel ? 'other' : ''),
					otherGrade: preferredGradeLabel,
			...
			showToast(
				'success',
				response?.data?.message || 'Teacher profile updated successfully.',
			);
		} catch (error) {
			showToast(
				'error',
	...
	const openDeleteConfirmation = () => {
		if (!deleteForm.password.trim()) {
			showToast(
				'error',
				'Re-enter your password in this section to delete your account.',
			);
			return;
		}
		...
		return (
			<form onSubmit={handleTeacherSubmit} noValidate>
				...
				<FieldRow
					...
					hint='Select from classes or choose Other'
				>
					<select
						id='selectedClassId'
					...
					>
						...
						{teacherForm.selectedClassId &&
							teacherForm.selectedClassId !== 'other' &&
							!hasSelectedClass && (
								<option value={teacherForm.selectedClassId}>
				...
				<FieldRow
					label='Bio'
					htmlFor='bio'
					hint='Short intro for students and parents'
				>
					<TextArea
						id='bio'
						name='bio'
				...
				<FieldRow
					label='Office hours'
					htmlFor='officeHours'
					hint='Office or online availability'
				>
					<TextInput
						id='officeHours'
						name='officeHours'
		...
		<div className='py-5'>
			<div className='rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-5'>
				...
				<p className='mt-2 text-sm text-[var(--color-text-secondary)] leading-relaxed'>
					Your parent and linked student details stay in the dedicated Parent
					Profile section.
				</p>
				<button
					type='button'
